...
* **Reviewed-file marks:** `m` toggles `DiffView::reviewed_files` (a `HashSet` of diff-relative file paths) for the selected file, or for every file below the selected directory; `M` toggles `DiffView::hide_reviewed`. Both are purely local — neither GitLab nor GitHub's "viewed" state is synced.
  ...
  - Marks persist in `ProjectCache::reviewed_files` (`mr_iid → {path → post-image blob SHA}`), written via `App::store_reviewed_files_for_mr` + `save_cache` on every toggle and re-seeded by `DiffView::restore_review_state` on `DiffFetched`. Restore drops a mark whose path left the diff, **and** one whose blob SHA moved — a mark that outlives its content hides the very file that needs re-reading, muted and folded behind its parent. Both SHAs must be known to unmark: an absent `index` line means the content did not change (pure rename, mode-only edit) or the backend cannot supply SHAs, and neither justifies discarding review work. `App::hide_reviewed_files` carries the filter across diff opens within a session.
  - Both diff entry points — the `view_diff` keybinding in [handlers/tabs.rs](src/handlers/tabs.rs) and the post-action refresh in [main.rs](src/main.rs) — go through `fetch::spawn_fetch_mr_diff`, which calls `Backend::get_mr_diff`. They each used to spawn the CLI inline as a bare `glab mr diff <iid>`, bypassing the backend: no `-R <project>` (so the diff came from the process's working directory rather than the selected repo) and no `--raw` (so no `index` lines, so no change detection on GitLab, while `Backend::get_mr_diff` sat unused). Never fetch a diff outside that helper.
  - The SHAs come from the diff's `index <old>..<new>` lines, parsed by `parse_index_post_image` into `DiffView::file_digests`. `GlabBackend::get_mr_diff` therefore passes **`--raw`**: GitLab's default rendering has no `diff --git` header, no `a/`/`b/` prefixes and no `index` lines, only ANSI colour. It falls back to the rendered form if `--raw` fails, so an older `glab` still shows diffs — without change detection. `gh pr diff` already emits the git format.
